...
public:
	...
	Controller(const char* const name) :
		BannerBase(name) {}
	void operator()
		(SharedResource& sharedResource, ControllerResource& controllerResource)
	{
...
int main()
{
	...
	std::future<void> taskController =
		std::async(std::launch::async, controller, std::ref(sharedResource),
			std::ref(controllerResource));
